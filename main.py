...
for i in range(100000):
    ...
    while True:
        ...
        buffer.store(state.cpu(), action, reward, next_state, done)
        if done or terminal:
            break
        state = next_state
    ...
    if len(buffer) > 1000:
        ...
        if i % 100 == 0:
            print(f"Iteration: {i}, Avg Position: {np.mean(positions):.4f}, Max Position: {max(positions):.4f}, Q_Loss: {q_loss.item():.4f}, P_Loss: {policy_loss.item():.4f}")
            positions = []
            q_target_net.load_state_dict(q_net.state_dict())
            policy_target_net.load_state_dict(policy_net.state_dict())

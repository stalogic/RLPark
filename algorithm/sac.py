...
class SAC(OffPolicyRLModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.policy_optimizer = torch.optim.Adam(
            self.policy_net.parameters(), lr=self.kwargs.get("policy_lr", lr)
        )
        self.q1_optimizer = torch.optim.Adam(
            self.q1_net.parameters(), lr=self.kwargs.get("value_lr", lr)
        )
        self.q2_optimizer = torch.optim.Adam(
            self.q2_net.parameters(), lr=self.kwargs.get("value_lr", lr)
        )
        ...
        self.log_alpha = torch.tensor(
            np.log(0.01), dtype=torch.float32, requires_grad=True, device=device
        )
    ...
    def take_action_with_mask(self, state, mask: list | np.ndarray) -> int:
        if not isinstance(mask, (list, np.ndarray)) or len(mask) != self.action_dim:
            raise TypeError(
                f"mask must be list or np.ndarray with length {self.action_dim}"
            )
    ...
    def predict_action_with_mask(self, state, mask: list | np.ndarray) -> int:
        ...
        with torch.no_grad(), self.eval_mode():
            state = torch.tensor(state, dtype=torch.float32).to(self.device)
            action_prob = self.policy_net(state)
            action_prob = action_prob * torch.tensor(mask, dtype=torch.float32).to(
                self.device
            )
    ...
    def _calc_target(self, rewards, next_states, dones):
        with torch.no_grad(), self.eval_mode():
            ...
            entropy = -torch.sum(
                next_action_prob * next_log_action_prob, dim=1, keepdim=True
            )
            q1_value = self.target_q1_net(next_states)
            q2_value = self.target_q2_net(next_states)
            min_q_value = torch.sum(
                next_action_prob * torch.min(q1_value, q2_value), dim=1, keepdim=True
            )
    ...
    def update(self) -> None:
        ...
        alpha_loss.backward()
        torch.nn.utils.clip_grad_norm_(
            self.log_alpha, self.kwargs.get("alpha_max_grad_norm", 0.3)
        )

...
import torch
import numpy as np
from .util import OffPolicyRLModel, DeterministicPolicyNetwork, DiscreteDeterministicPolicyNetwork, ContinuousQValueNetwork

class DDPG(OffPolicyRLModel):
    
    def __init__(self, state_dim_or_shape, action_dim_or_shape, hidden_dims=(32,), conv_layers=((32, 3),), batch_size=128, action_bound=1, sigma=0.05, lr=1e-3, gamma=0.99, capacity=10000, device="cpu", **kwargs) -> None:
        ...
        self.qvalue_optimizer = torch.optim.Adam(self.qvalue_net.parameters(), lr=self.kwargs.get('critic_lr', lr))
        ...
        self.qvalue_lr_scheduler = torch.optim.lr_scheduler.StepLR(self.qvalue_optimizer, step_size=self.kwargs.get("scheduler_step_size", 100), gamma=0.955)
    ...
    def update(self) -> None:
        ...
        with torch.no_grad(), self.eval_mode():
            next_q_values = self.target_qvalue_net(next_states, self.target_policy_net(next_states))
...
class DiscreteDDPG(OffPolicyRLModel):
    ...
    def gumbel_softmax_sample(logits, temperature):
        ...
        y = logits + DiscreteDDPG.sample_gumbel(logits.shape, tens_type=type(logits.data)).to(
            logits.device)
    ...
    def __init__(self, state_dim_or_shape, action_dim_or_shape, hidden_dims=(32,), conv_layers=((32, 3),), batch_size=128, lr=1e-3, gamma=0.99, capacity=10000, device="cpu", **kwargs) -> None:
        super().__init__(state_dim_or_shape, action_dim_or_shape, capacity)
        ...
        self.policy_net = DiscreteDeterministicPolicyNetwork(self.state_shape, self.action_shape, hidden_dims=hidden_dims, conv_layers=conv_layers, **kwargs)
        self.qvalue_net = ContinuousQValueNetwork(self.state_shape, self.action_shape, hidden_dims=hidden_dims, conv_layers=conv_layers, **kwargs)
        self.target_policy_net = copy.deepcopy(self.policy_net)
        self.target_qvalue_net = copy.deepcopy(self.qvalue_net)
    ...
    def update(self) -> None:
        ...
        policy_loss = -torch.mean(self.qvalue_net(states, DiscreteDDPG.gumbel_softmax(action_logits)))
        self.policy_optimizer.zero_grad()
        policy_loss.backward()
        torch.nn.utils.clip_grad_norm_(self.policy_net.parameters(), 0.5)
        self.policy_optimizer.step()
